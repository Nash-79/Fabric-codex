...
`src/components/` reader components, `src/styles.css`, `public/diagrams/*.svg`.
...
| 5 | **7 SVGs carry fixed `width`/`height`** alongside `viewBox`, defeating fluid scaling. Strip the fixed attributes, keep `viewBox`. | `public/diagrams/*.svg` |
...
npm run validate:diagrams      # if you touched public/diagrams/
```
