...
- **Graphic Metaphors:** Choose from the 6 core archetypes:
  1. _Pipeline Flow & Medallion Rail_ (left-to-right continuous ribbon with stage hubs and payload badges).
  2. _Decision Tree & Trade-Off Matrix_ (diamond/hex decision hubs with YES/NO/FALLBACK condition pills).
  3. _Engine Internals Cutaway Stack_ (layered vertical execution stack with SIMD/vectorized blocks and fallback trapdoors).
  4. _State Transition & Memory Ladder_ (color-graduated thermometers from Cold to Hot with latency gauges).
  5. _Storage Anatomy & Object Model_ (exploded log/file hierarchy with 1:1 segment mapping).
  6. _Trap vs Remedy Split_ (side-by-side comparison with red breakages vs green governed flows).
