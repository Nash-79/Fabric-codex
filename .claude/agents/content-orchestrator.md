...
1. **Snapshot the work.** Group work into:
   ...
   - approved article ideas: `.queue[]` where `kind="idea"` and `status="claimed"` (an admin
     approved it in Settings → Article Ideas). These carry structured rationale in `notes`
     (angle, signal_type, supporting capability/roadmap ids, suggested diagrams) generated by the
     Lovable AI Gateway from roadmap/coverage/backlog/staleness signals — treat an approved idea
     as a strong, human-vetted signal for which topic to draft next, and read its rationale before
     drafting instead of starting from a blank slate;
