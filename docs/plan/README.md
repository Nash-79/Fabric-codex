...
- [ ] **D4: 0 of 3,052 claims embedded — code complete, one command from done.** `scripts/generate-embeddings.mjs` had never run: it imported `dotenv` (not a dependency), selected non-existent columns, silently capped at PostgREST's 1000-row default, and its `main()` only printed a tip without writing anything. Rewritten with keyset/streaming paging (OFFSET paging over a `WHERE embedding IS NULL` filter is wrong — embedded rows leave the filtered set, so each page skipped that many unprocessed rows), 6-way concurrency, dimension checking, `--force`/`--dry-run`. **The blocker was permissions, and is now designed around:** the anon key can read `claims` but not write them, and an RLS-blocked UPDATE returns _no error and no rows_ — so the script reported "200 embedded" while the database stayed at 0. The service-role key is server-only and Lovable-managed, so it cannot be handed to a laptop script. **Architecture (chosen 2026-08-24): the laptop computes, the server writes.** Ollama generates vectors locally (free, unmetered, consistent with "LLM work happens on your laptop"); the script POSTs batches of ≤200 to `POST /api/public/hooks/claim-embeddings`, which writes them with `supabaseAdmin`. Vectors cross the wire; no secret does. Shared validation lives in `src/lib/claim-embeddings.server.ts` (whole-batch validation before any write, so one bad vector cannot half-embed the corpus; `.select("id")` on every update so a non-matching row is _reported_, never counted as a silent success) and is exposed twice: `writeClaimEmbeddings` (admin server function, audited) and the token-authed hook route for the script. 11 unit tests including the false-success regression; auth gate and validation path verified end-to-end against a dev server (401 without/with a bad token; 400 with precise messages for empty batch and wrong dimensions). **Verified 2026-08-24: 0/3,052 still, and the write endpoint is not live yet** (`https://fabric-atlas.lovable.app/api/public/hooks/claim-embeddings` returns 404 -- the route exists only in this working tree, not in the deployed app). The script now probes that endpoint before doing any embedding work and reports exactly which of the two remaining steps is outstanding (404 = not deployed, 401 = wrong token) instead of grinding through Ollama calls first. Secrets now have a proper home: `.env.local` (gitignored; `.env` stays committed and public-values-only) with `.env.local.example` documenting the two variables needed. **To finish, in order:** (1) commit + deploy so the route ships, (2) set `FABRIC_ATLAS_AGENT_READ_TOKEN` in `.env.local` to the value configured on the server, (3) `node scripts/generate-embeddings.mjs` (~5 min, defaults `FABRIC_ATLAS_APP_URL` to the production deployment already).
