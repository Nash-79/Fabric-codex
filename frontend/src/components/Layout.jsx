...
export const NAV = [
  ["/", "Overview"], ["/topics", "Topics"], ["/search", "Search"],
  ["/registry", "Registry"], ["/sources", "Sources"], ["/designs", "Designs"],
  ["/learn", "Learn"], ["/help", "Help"], ["/author", "Author"],
];
        ...
        <div style={{ display: "flex", borderBottom: "1px solid " + c.line, overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
          {NAV.map(([to, label]) => (
            <NavLink key={to} to={to} end={to === "/"}
              style={({ isActive }) => ({ fontFamily: sans, fontSize: 13, fontWeight: isActive ? 600 : 500, textDecoration: "none", color: isActive ? c.text : c.muted, borderBottom: "2.5px solid " + (isActive ? c.accent : "transparent"), padding: isMobile ? "12px 12px" : "12px 14px", marginBottom: -1, whiteSpace: "nowrap", minHeight: 44, display: "flex", alignItems: "center" })}>
              {label}
            </NavLink>
          ))}
        </div>
