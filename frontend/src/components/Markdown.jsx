...
export const CiteChip = ({ tag, title }) => (
  <span title={title} style={{ fontFamily: mono, fontSize: 11, color: c.accentText, background: c.accentSoft, border: "1px solid " + c.accentDim, borderRadius: 4, padding: "0 4px", margin: "0 1px", cursor: title ? "help" : "inherit" }}>{tag}</span>
);
    ...
    return React.Children.map(children, (child) =>
      typeof child === "string"
        ? child.split(/(\[S\d+\])/g).map((p, i) => {
            ...
            return <CiteChip key={i} tag={tag} title={src ? `${src.title} (T${src.tier})` : undefined} />;
          })
        : child
    );
  };
...
const heading = (cite, size, upper = false) => ({ children }) => {
  const text = React.Children.toArray(children).filter((x) => typeof x === "string").join(" ");
  const id = text ? text.toLowerCase().replace(/\[s\d+\]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") : undefined;
  return (
    <div id={id} style={{ color: c.accentText, fontFamily: sans, fontWeight: 600, fontSize: size, letterSpacing: upper ? 0.3 : 0, textTransform: upper ? "uppercase" : "none", margin: "18px 0 6px" }}>{cite(children)}</div>
  );
};
...
function buildComponents(legendByTag) {
  ...
  return {
    h1: heading(cite, 16), h2: heading(cite, 13, true), h3: heading(cite, 13, true),
    h4: heading(cite, 12.5, true), h5: heading(cite, 12.5, true), h6: heading(cite, 12.5, true),
    ...
    img: ({ src, alt }) => (
      <img src={src} alt={alt}
        style={{ maxWidth: "100%", borderRadius: 6, background: c.diagramBg, border: "1px solid " + c.lineSoft, margin: "8px 0" }}
...
export function Md({ text, legend }) {
  if (!text) return null;
  const legendByTag = legend
    ? Object.fromEntries(legend.map((s) => [s.tag, s]))
    : null;
