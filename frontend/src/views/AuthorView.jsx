...
export default function AuthorView() {
  const steps = [
    ["1 · Queue", "Sources tab → Add a source", "submit URLs from the UI — they land in the ingestion queue for the curator"],
    ["2 · Ingest", "/ingest-batch (or /ingest <url> tier=1)", "knowledge-curator pulls the queue, extracts claims + tags + image refs → content/sources/*.json → POST /sources/ingest"],
    ["3 · Diagram", "/diagram <capability-id>", "diagram-author draws an ORIGINAL Mermaid/SVG → content/diagrams/* → POST /assets"],
    ["4 · Verify", "Registry tab → Verify", "human approval — pending claims become verified"],
    ["5 · Publish a topic", "/publish-topic <topic-slug>", "blog-author composes a cited article from verified claims; validation-reviewer checks it → Topics tab"],
    ["6 · Design", "/design <scenario>", "solution-architect writes a cited architecture → content/designs/*.md → POST /designs"],
    ["7 · Validate", "/validate <design-id>", "validation-reviewer reasons locally, posts issues; server adds citation + freshness → confidence"],
    ["8 · Teach", "/lesson <capability> <level>", "learning-author writes a grounded lesson → content/lessons/*.md → Learn tab"],
    ["9 · Maintain", "/drift <source-key>", "source-drift-analyst re-extracts, supersedes changed claims, flags affected designs and blogs"],
  ];
  return (
