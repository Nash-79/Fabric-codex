...
export default function DesignsView() {
  ...
  const validate = async () => {
    setBusy(true);
    try {
      await api(`/designs/${open.id}/validate`, { method: "POST", body: JSON.stringify({}) });
      await view(open.id);
      refresh();
    } finally {
      setBusy(false);
  ...
  if (!designs.length)
    return (
      <Empty>
        No designs yet. Run <Code>/design &lt;scenario&gt;</Code> in Claude Code.
      </Empty>
    );
    ...
    <div
      style={
        isMobile
          ...
          : {
              display: "grid",
              gridTemplateColumns: open ? "minmax(260px, 320px) 1fr" : "1fr",
              gap: 16,
            }
      }
    >
          ...
          {designs.map((d) => (
            <button
              ...
              style={{
                textAlign: "left",
                cursor: "pointer",
                background: open?.id === d.id ? c.accentSoft : c.panel,
                border: "1px solid " + (open?.id === d.id ? c.accent : c.line),
                borderRadius: 8,
                padding: 12,
                boxShadow: c.shadow,
              }}
            >
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 6, color: c.text }}>
                {d.title || d.scenario.slice(0, 50)}
              </div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                ...
                {d.confidence != null && (
                  <Chip color={c.accentText}>{Math.round(d.confidence * 100)}%</Chip>
                )}
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <Btn small primary onClick={validate} disabled={busy}>
                {busy ? "Validating…" : "Run validation"}
              </Btn>
              {!isMobile && (
                <Btn small onClick={closeDetail}>
                  Close
                </Btn>
              )}
            </div>
