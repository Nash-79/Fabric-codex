  ...
  if (!files.length) {
    return (
      <Empty>
        ...
        <Code>/lesson &lt;capability&gt; &lt;Beginner|Intermediate|Expert&gt;</Code> — lessons are
        grounded only in approved claims (Beginner=L1–L2, Intermediate=L3, Expert=L4–L5).
      </Empty>
    );
  }
    ...
    <div
      style={
        isMobile
          ...
          : {
              display: "grid",
              gridTemplateColumns: open ? "minmax(240px, 300px) 1fr" : "1fr",
              gap: 16,
            }
      }
    >
          ...
          {files.map((f) => (
            <button
              ...
              style={{
                textAlign: "left",
                cursor: "pointer",
                background: open?.name === f.name ? c.accentSoft : c.panel,
                border: "1px solid " + (open?.name === f.name ? c.accent : c.line),
                borderRadius: 8,
                padding: 12,
                boxShadow: c.shadow,
              }}
