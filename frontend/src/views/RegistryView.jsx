...
export default function RegistryView() {
  ...
  const dismissDup = (id) =>
    withActioning(id, () => api(`/claims/${id}/dismiss`, { method: "POST" }));
  ...
  const verifyAllShown = async () => {
    ...
    try {
      ...
      const res = await api("/claims/verify-bulk", {
        method: "POST",
        body: JSON.stringify({ claim_ids: ids }),
      });
      showUndoToast(
        `Verified ${res.verified} claim${res.verified !== 1 ? "s" : ""}`,
        res.verified_ids,
      );
    } catch (e) {
      setErr(e.message);
  ...
  const rejectAllShown = async () => {
    ...
    try {
      const ids = pendingShown.map((cl) => cl.id);
      const res = await api("/claims/reject-bulk", {
        method: "POST",
        body: JSON.stringify({ claim_ids: ids }),
      });
      showUndoToast(
        `Rejected ${res.rejected} claim${res.rejected !== 1 ? "s" : ""}`,
        res.rejected_ids,
      );
    } catch (e) {
      setErr(e.message);
  ...
  return (
    <div>
      ...
      {total === 0 && (
        <Empty>
          The knowledge base is empty. Author content with the agents, then publish:
          <br />
          ...
          {Object.entries(tags).map(([t, n]) => (
            <button
              ...
              style={{
                ...
                fontFamily: mono,
                fontSize: 11,
                color: tagFilter === t ? c.onAccent : c.accentText,
                background: tagFilter === t ? c.accent : c.accentSoft,
                border: "1px solid " + (tagFilter === t ? c.accent : c.accentDim),
                borderRadius: 12,
                padding: "2px 9px",
              }}
            >
          ...
          >
            {CAPABILITIES.filter((x) => x.area === area).map((x) => {
              ...
              return (
                <button
                  key={x.id}
                  onClick={() => setCap(cap === x.id ? null : x.id)}
                  style={{
                    textAlign: "left",
                    cursor: "pointer",
                    background: cap === x.id ? c.accentSoft : c.panel,
                    border: "1px solid " + (cap === x.id ? c.accent : c.line),
                    borderRadius: 8,
                    padding: "11px 12px",
                    boxShadow: c.shadow,
                  }}
                >
                  ...
                  >
                    ...
                    <span
                      style={{ fontFamily: mono, fontSize: 11, color: n ? c.accentText : c.faint }}
                    >
                      {n}
                    </span>
                  </div>
                        ...
                        <div key={d.n} style={{ flex: 1 }} title={`${d.short} ${d.label}: ${k}`}>
                          <div
                            style={{
                              height: 5,
                              borderRadius: 2,
                              background: k ? c.accent : c.lineSoft,
                              opacity: k ? Math.min(0.4 + k * 0.2, 1) : 1,
                            }}
                          />
                          <div
                            style={{
            ...
            <div style={{ borderTop: "1px solid " + c.lineSoft }}>
              ...
              >
                ...
                <select
                  ...
                  style={{
                    fontFamily: mono,
                    fontSize: 11,
                    color: capFilter ? c.accentText : c.muted,
                    background: capFilter ? c.accentSoft : c.bg,
                    border: "1px solid " + (capFilter ? c.accent : c.line),
                    borderRadius: 6,
                    padding: "3px 7px",
                    cursor: "pointer",
                  }}
                >
                  ...
                  {actionCapIds.map((id) => (
                    <option key={id} value={id}>
                      {CAPABILITIES.find((x) => x.id === id)?.name || id}
                    </option>
                  ))}
                </select>
                {/* action-type chips */}
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                  {ACTION_TYPES.filter((a) => recentActions.some((ev) => ev.action === a)).map(
                    (a) => {
                      const active = actionFilter === a;
                      const chipColor =
                        a === "verified"
                          ? c.green
                          : a === "rejected" || a === "dismissed"
                            ? c.red
                            : c.amber;
                      return (
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 12,
                            padding: "2px 9px",
                            border: "1px solid " + (active ? chipColor : c.line),
              ...
              {filteredActions.length === 0 ? (
                ...
                filteredActions.map((ev) => {
                  const capName =
                    CAPABILITIES.find((x) => x.id === ev.capability_id)?.name || ev.capability_id;
                  const actionColor =
                    ev.action === "verified"
                      ? c.green
                      : ev.action === "rejected" || ev.action === "dismissed"
                        ? c.red
                        : c.amber;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {ev.text_snippet}
                          {ev.text_snippet.length >= 120 ? "…" : ""}
                        </div>
                        <div
                          style={{ display: "flex", gap: 6, marginTop: 3, alignItems: "center" }}
                        >
                          ...
                          <span style={{ fontFamily: mono, fontSize: 10, color: actionColor }}>
                            {ev.new_status}
                          </span>
                        </div>
                      </div>
      ...
      {(cap || tagFilter) && (
        ...
        >
              ...
              {pendingShown.length > 1 && (
                <>
                  <CountdownBtn
                    small
                    primary
                    onClick={verifyAllShown}
                    countdown={bulkConfirm === "verify"}
                  >
                    {bulkConfirm === "verify"
                      ? `Confirm verify all? (${pendingShown.length})`
                      : `Verify all (${pendingShown.length})`}
                  </CountdownBtn>
                  <CountdownBtn small onClick={rejectAllShown} countdown={bulkConfirm === "reject"}>
                    {bulkConfirm === "reject"
                      ? `Confirm reject all? (${pendingShown.length})`
                      : `Reject all (${pendingShown.length})`}
                  </CountdownBtn>
                </>
              )}
              {duplicates.length > 0 && (
                ...
                >
                  {dupOpen ? "Hide" : "Show"} {duplicates.length} duplicate
          ...
          {capAssets
            .filter((a) => a.kind === "generated" && a.path)
            .map((a) => (
              <div
                key={a.id}
                style={{ padding: "12px 14px", borderBottom: "1px solid " + c.lineSoft }}
              >
                <img
                  src={"/" + a.path.replace(/^\//, "")}
                  alt={a.caption}
          ...
          <div>
            {claims.length === 0 && (
              <div style={{ padding: 16, color: c.muted, fontSize: 13 }}>No claims match.</div>
            )}
            {claims.map((cl) => {
              ...
              return (
                ...
                >
                  <div style={{ display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap" }}>
                    ...
                    {cl.status === "verified" ? (
                      <Chip color={c.green}>✓ verified</Chip>
                    ) : cl.status === "pending" ? (
                  ...
                  <div
                    style={{ display: "flex", gap: 6, marginTop: 7, justifyContent: "flex-end" }}
                  >
                    {busy ? (
                      <span style={{ fontFamily: mono, fontSize: 12, color: c.muted }}>…</span>
                    ) : cl.status === "pending" ? (
                      <>
                        <Btn small primary onClick={() => verify(cl.id)}>
                          Verify
                        </Btn>
                        <CountdownBtn
                          small
                          onClick={() => rejectWithConfirm(cl.id)}
                          countdown={rejectConfirm.has(cl.id)}
                        >
                    ...
                    <Btn
                      ...
                      disabled={busy}
                      onClick={async () =>
                        !busy && setHistory(await api(`/claims/${cl.claim_key}/history`))
                      }
                    >
                      History
                    </Btn>
          ...
          {dupOpen && duplicates.length > 0 && (
            <div style={{ borderTop: "2px solid " + c.amber }}>
              ...
              >
                ...
                <span style={{ fontFamily: mono, fontSize: 11, color: c.faint }}>
                  Promote to re-enter verify queue · Dismiss to confirm as duplicate
                </span>
              </div>
              {duplicates.map((cl) => {
                const busy = actioning.has(cl.id);
                  ...
                  >
                    ...
                    >
                      {busy ? (
                        ...
                        <>
                          <Btn small primary onClick={() => promote(cl.id)}>
                            Promote
                          </Btn>
                          <Btn small onClick={() => dismissDup(cl.id)}>
                            Dismiss
                          </Btn>
                        </>
                      )}
      ...
      {undoToast && (
        <div
          key={undoToast.label + undoToast.claimIds[0]}
          style={{
            position: "fixed",
            bottom: 24,
            left: "50%",
        ...
        >
          ...
          <div style={{ height: 3, background: c.lineSoft }}>
            <div
              key={undoToast.claimIds.join(",")}
              style={{
                height: "100%",
                background: c.accent,
                animation: "cd-shrink 5s linear forwards",
