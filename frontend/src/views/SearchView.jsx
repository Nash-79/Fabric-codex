    ...
    <span style={{ fontSize: 13, color: c.text, lineHeight: 1.55 }}>
      {parts.map((p, i) => {
        ...
        return bold ? (
          <mark
            key={i}
            style={{ background: c.accentSoft, color: c.accentText, fontWeight: 600, padding: 0 }}
          >
            {p}
          </mark>
        ) : (
        ...
        {KINDS.map(([k, label]) => (
          <button
            ...
            style={{
              ...
              fontFamily: mono,
              fontSize: 11,
              color: kind === k ? c.onAccent : c.accentText,
              background: kind === k ? c.accent : c.accentSoft,
              border: "1px solid " + (kind === k ? c.accent : c.accentDim),
              borderRadius: 12,
              padding: "3px 11px",
            }}
          >
                      ...
                      >
                        ...
                        {r.kind === "claim" && r.capability_id && (
                          <Chip color={c.accentText}>
                            {CAPABILITIES.find((x) => x.id === r.capability_id)?.name ||
                              r.capability_id}
                          </Chip>
                        )}
                        {r.kind === "claim" && r.status && (
                          <Chip color={r.status === "verified" ? c.green : c.amber}>
                            {r.status}
                          </Chip>
                        )}
                        {r.kind === "source" && r.tier && (
                          <Chip color={TIER_COLORS[r.tier]}>T{r.tier}</Chip>
                        )}
                        {r.kind === "blog" && r.status && (
                          <Chip
                            color={
                              r.status === "validated"
                                ? c.green
                  ...
                  const to =
                    r.kind === "blog"
                      ...
                      : r.kind === "topic"
                        ? `/topics/${r.ref}`
                        : r.kind === "claim"
                          ? `/registry?cap=${r.capability_id || ""}`
                          : "/sources";
                  return (
                    <Link
                      key={r.kind + r.ref + (r.id || "")}
                      to={to}
