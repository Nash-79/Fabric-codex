...
function SubmitSourceForm({ onSubmitted }) {
  ...
  return (
    ...
    >
      ...
      >
        <Btn small primary disabled={busy || !url.trim()}>
          {busy ? "Submitting…" : "Queue for ingestion"}
        </Btn>
      ...
      {visible.map((q) => (
        ...
        >
          ...
          <span style={{ display: "flex", gap: 6 }}>
            {q.status === "failed" && (
              <Btn small onClick={() => onAction(q.id, "requeue")}>
                Retry
              </Btn>
            )}
            {(q.status === "queued" || q.status === "failed") && (
              <Btn small onClick={() => onAction(q.id, "dismiss")}>
                Dismiss
              </Btn>
            )}
          </span>
...
export default function SourcesView() {
  ...
  const refresh = useCallback(() => {
    ...
    api("/claims?status=pending")
      .then((rows) => {
        const by = {};
        rows.forEach((cl) => {
          by[cl.source_id] = (by[cl.source_id] || 0) + 1;
        });
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      {!sources.length && (
        <Empty>
          No sources yet. Queue a URL above, then run <Code>/ingest-batch</Code> in Claude Code and
          publish.
        </Empty>
      )}
      {sources.map((s) => {
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              {pending > 0 && (
                <span style={{ marginLeft: "auto" }}>
                  <Btn small primary onClick={() => verifySource(s.id)}>
                  ...
                  >
                    {a.kind === "generated" && a.path ? (
                      <img
                        src={"/" + a.path.replace(/^\//, "")}
                        alt={a.caption}
                        style={{ maxWidth: "100%", borderRadius: 4, background: c.diagramBg }}
                        onError={(e) => {
