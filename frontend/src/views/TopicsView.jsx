...
function TopicNode({ node, depth, index, selectedSlug, expanded, toggle }) {
  ...
  return (
    <div
      style={
        isSection && index > 0
          ? { marginTop: 10, paddingTop: 8, borderTop: "1px solid " + c.lineSoft }
          : undefined
      }
    >
      <div style={{ display: "flex", alignItems: "center", gap: 4, paddingLeft: depth * 16 }}>
        {node.children.length > 0 ? (
          <button
            ...
            style={{
              ...
              transform: isOpen ? "none" : "rotate(0deg)",
            }}
          >
            {isOpen ? "▾" : "▸"}
          </button>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 6,
            padding: isSection ? "8px 9px" : "7px 9px",
            display: "flex",
            alignItems: "center",
            gap: 8,
          }}
        >
          <span
            style={{
              ...
              fontWeight: isSection ? 700 : selected ? 600 : 500,
              color: c.text,
              letterSpacing: isSection ? 0.2 : 0,
          ...
          <span style={{ marginLeft: "auto", display: "flex", gap: 5, alignItems: "center" }}>
            {node.verified_claims > 0 && (
              <span
                title={`${node.verified_claims} verified claims`}
                style={{ fontFamily: mono, fontSize: 10, color: c.accentText }}
              >
                {node.verified_claims}
              </span>
            )}
            {node.blog && (
              <span
                title={`Article: ${node.blog.status}`}
                style={{
                  width: 7,
                  height: 7,
                  borderRadius: "50%",
                  background: STATUS_COLORS[node.blog.status] || c.faint,
      ...
      {depth <= 4 && node.children.length > 0 && (
        <div
          style={{
            maxHeight: isOpen ? node.children.length * 320 : 0,
            opacity: isOpen ? 1 : 0,
            overflow: "hidden",
...
export default function TopicsView() {
  ...
  useEffect(() => {
    api("/topics?include_counts=true")
      .then((rows) => {
        setTopics(rows);
  ...
  useEffect(() => {
    ...
    api(`/topics/${slug}`)
      .then((d) => {
        ...
        Promise.all(caps.map((id) => api(`/assets?capability=${id}`).catch(() => []))).then(
          (lists) => {
            const seen = new Set();
            const flat = [];
  ...
  if (!topics.length) {
    return (
      <Empty>
        No topics yet. Seed the topic tree with <Code>python scripts/import_content.py</Code> (reads
        content/topics.json).
      </Empty>
    );
  }
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <span style={{ marginLeft: "auto", fontSize: 12, color: c.faint }}>
            click any component to open its topic
          </span>
        </button>
        <div
          style={{
            maxHeight: mapOpen ? 1400 : 0,
            overflow: "hidden",
      ...
      <div
        style={
          isMobile
            ? {}
            : { display: "grid", gridTemplateColumns: "minmax(240px, 320px) 1fr", gap: 18 }
        }
      >
        ...
        {showDetail ? (
              ...
              {detail.blog ? (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <span style={{ fontWeight: 600, fontSize: 14.5, color: c.text }}>
                        {detail.blog.title}
                      </span>
                      <Chip color={STATUS_COLORS[detail.blog.status] || c.muted}>
                        {detail.blog.status}
                      </Chip>
                      {detail.blog.ready_to_share && <Chip color={c.green}>✓ ready</Chip>}
                      {detail.blog.confidence != null && (
                        <Chip color={c.accentText}>
                          {Math.round(detail.blog.confidence * 100)}%
                        </Chip>
                      )}
                    </div>
                    {detail.blog.summary && (
                    ...
                    <div
                      style={{ fontFamily: mono, fontSize: 11, color: c.accentText, marginTop: 8 }}
                    >
                      Read the article →
                    </div>
                  </div>
                        ...
                        >
                          ...
                          >
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                              ...
                              {flat?.blog && (
                                <span
                                  title={`Article: ${flat.blog.status}`}
                                  style={{
                                    width: 7,
                                    height: 7,
                                    borderRadius: "50%",
                                    background: STATUS_COLORS[flat.blog.status] || c.faint,
                                    display: "inline-block",
                                  }}
                                />
                              )}
          ...
          !isMobile && (
            <Empty>
              Pick a topic to see its article, mapped capabilities, diagrams, and sub-topics. Dots
              mark topics with a published article; numbers are verified claims behind it.
            </Empty>
          )
        )}
      </div>
