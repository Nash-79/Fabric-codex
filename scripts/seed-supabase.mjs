...
const CAP = {
  "fabric-platform": ["Fabric Platform", "indigo"], capacity: ["Capacity & Cost", "amber"],
  purview: ["Governance & Purview", "violet"], onelake: ["OneLake", "teal"],
  lakehouse: ["Lakehouse", "teal"], mirroring: ["Mirroring", "teal"], spark: ["Spark", "rose"],
  "data-factory": ["Data Factory", "rose"], "dataflow-gen2": ["Dataflow Gen2", "rose"],
  warehouse: ["Warehouse", "yellow"], polaris: ["Polaris SQL Engine", "yellow"],
  "sql-database": ["SQL Database in Fabric", "yellow"], "direct-lake": ["Direct Lake", "indigo"],
  "semantic-model": ["Semantic Model", "indigo"], "power-bi": ["Power BI", "indigo"],
  rti: ["Real-Time Intelligence", "rose"], "eventhouse-kql": ["Eventhouse / KQL", "rose"],
  "fabric-data-agent": ["Fabric Data Agent", "violet"], "fabric-iq": ["Fabric IQ", "violet"],
...
for (const { data: s } of sources) (s.claims ?? []).forEach(c => c.capability_id && capIds.add(c.capability_id));

const must = async (label, p) => { const { error, data } = await p; if (error) { console.error(label, error); throw error; } return data; };

// Capabilities
const capRows = [...capIds].map(id => ({ id, name: CAP[id]?.[0] ?? id, accent: CAP[id]?.[1] ?? "teal", description: "" }));
if (capRows.length) await must("caps", sb.from("capabilities").upsert(capRows, { onConflict: "id" }));
console.log("capabilities:", capRows.length);

// Topics
const ordered = [...topics].sort((a, b) => (a.parent_slug ? 1 : 0) - (b.parent_slug ? 1 : 0));
for (const t of ordered) {
  await must(`topic ${t.slug}`, sb.from("topics").upsert({
    slug: t.slug, parent_slug: t.parent_slug ?? null, name: t.name,
    description: t.description ?? "", sort_order: t.order ?? 0,
  }, { onConflict: "slug" }));
}
...
const tcRows = [];
for (const t of topics) for (const id of t.capability_ids ?? []) tcRows.push({ topic_slug: t.slug, capability_id: id });
...
for (const { slug, data: s } of sources) {
  const ins = await must(`src ${slug}`, sb.from("sources").upsert({
    slug, url: s.url, title: s.title, tier: s.tier ?? 6, tags: s.tags ?? [], summary: s.summary ?? "",
  }, { onConflict: "slug" }).select("id").single());
  slugToId.set(slug, ins.id);
  await sb.from("claims").delete().eq("source_id", ins.id);
  const rows = (s.claims ?? []).filter(c => c.capability_id && capIds.has(c.capability_id)).map(c => ({
    source_id: ins.id, capability_id: c.capability_id, text: c.text,
    depth: c.depth ?? 2, type: c.type ?? "fact", tags: c.tags ?? [],
  }));
  if (rows.length) { await must(`claims ${slug}`, sb.from("claims").insert(rows)); claimCount += rows.length; }
}
...
const diagRows = diagrams.map(d => {
  ...
  return { slug, path: `/diagrams/${slug}.svg`, caption: d.caption ?? "", kind: d.kind ?? "architecture", topic_slug: null };
});
if (diagRows.length) await must("diagrams", sb.from("diagrams").upsert(diagRows, { onConflict: "slug" }));
console.log("diagrams:", diagRows.length);

// Help
await sb.from("help_docs").delete().neq("slug", "__never__");
const helpDir = "content/help";
const helpRows = existsSync(helpDir) ? readdirSync(helpDir).filter(f => f.endsWith(".md")).map(f => {
  const body = readFileSync(join(helpDir, f), "utf8");
  const base = f.replace(/\.md$/, "");
  const m = base.match(/^(\d+)-(.+)$/);
  const order = m ? parseInt(m[1]) : 0;
  const slug = m ? m[2] : base;
  const title = (body.split("\n").find(l => l.startsWith("#")) ?? slug).replace(/^#+\s*/, "");
  return { slug, title, body_md: body, sort_order: order };
}) : [];
if (helpRows.length) await must("help", sb.from("help_docs").upsert(helpRows, { onConflict: "slug" }));
