import { depthMeta, maturityMeta, tierMeta, type Maturity } from "@/lib/fabric-theme";
...
export function MaturityBadge({ maturity }: { maturity?: string | null }) {
  const m = maturityMeta[(maturity as Maturity) ?? "ga"] ?? maturityMeta.ga;
  return (
    <span className={`rounded-md border px-1.5 py-0.5 text-[10px] font-semibold ${m.cls}`}>
      {m.label}
    </span>
