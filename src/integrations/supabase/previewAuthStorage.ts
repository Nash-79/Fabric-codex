...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      // `finish` (declared next) must be able to `clearTimeout(timer)` before the timer id
      // exists, so this can't be a single `const` initializer despite being assigned only once.
