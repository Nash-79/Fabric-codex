...
import { searchAll, listContentItemsPage } from "@/lib/atlas.functions";
...
function SearchPage() {
  ...
  // Cursor-paginated (WP3.2): the library previously fetched everything and dropped all but the
  // first 40 client-side, so most of the corpus was unreachable with no indication it existed.
  const library = useInfiniteQuery({
    queryKey: ["content-items", "library", kind, capability],
    queryFn: ({ pageParam }) =>
      listContentItemsPage({
        data: { kind, capabilityId: capability, limit: LIBRARY_PAGE_SIZE, cursor: pageParam },
      }),
            ...
            {library.hasNextPage && (
              <div className="mt-4 flex justify-center">
                <button
                  ...
                  className="rounded-md border border-border bg-card px-4 py-2 text-xs font-semibold text-foreground transition hover:bg-accent disabled:opacity-50"
                >
                  {library.isFetchingNextPage ? "Loading…" : "Load more"}
                </button>
              </div>
            )}
            {libraryItems.length > 0 && !library.hasNextPage && (
              <div className="mt-4 text-center text-xs text-muted-foreground">
                {libraryItems.length} item{libraryItems.length === 1 ? "" : "s"} — end of library
              </div>
            )}
          </div>
        )}
