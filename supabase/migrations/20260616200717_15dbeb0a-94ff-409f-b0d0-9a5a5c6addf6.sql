-- =============== 20260616130000_content_documents_and_guards ===============
...
CREATE UNIQUE INDEX IF NOT EXISTS blogs_one_active_slug ON public.blogs (slug) WHERE active;
...
-- =============== 20260616143000_admin_settings_cms ===============
...
CREATE OR REPLACE FUNCTION public.admin_record_event(
  _action text, _target_type text DEFAULT '', _target_id text DEFAULT '', _metadata jsonb DEFAULT '{}'::jsonb
...
BEGIN
  ...
  END LOOP;
  INSERT INTO public.admin_audit_events (actor_id, action, target_type, target_id, metadata)
  VALUES (auth.uid(), 'user.roles_changed', 'user', _user_id::text, jsonb_build_object('roles', _roles));
END; $$;

CREATE OR REPLACE FUNCTION public.admin_approve_user(
  _user_id uuid, _roles public.app_role[] DEFAULT ARRAY['user']::public.app_role[]
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN RAISE EXCEPTION 'Forbidden'; END IF;
  IF _user_id IS NULL THEN RAISE EXCEPTION 'user_id is required'; END IF;
  ...
  ON CONFLICT (id) DO UPDATE SET status='approved', approved_at=now(), approved_by=auth.uid(),
  ...
  INSERT INTO public.admin_audit_events (actor_id, action, target_type, target_id, metadata)
  VALUES (auth.uid(), 'user.approved', 'user', _user_id::text,
...
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN RAISE EXCEPTION 'Forbidden'; END IF;
  IF _user_id IS NULL THEN RAISE EXCEPTION 'user_id is required'; END IF;
  IF _user_id = auth.uid() THEN RAISE EXCEPTION 'Admins cannot suspend themselves'; END IF;
  UPDATE public.profiles SET status='suspended', suspended_at=now(), suspended_by=auth.uid(), updated_at=now()
  WHERE id = _user_id;
  DELETE FROM public.user_roles WHERE user_id = _user_id;
...
-- =============== 20260616150000_profile_approval_access ===============
